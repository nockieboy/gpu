void sd_card_initialisation {

    // FNIRSI_1013D_Firmware version
    ...
    sd_command.cmdidx    = 08;
    sd_command.resp_type = 21;

    sd_command.cmdarg    = 00 00 01 55; //31:12 reserved (0x00000), 11:8 supply voltage (0x1 = 2.7 - 3.6V), 7:0 check pattern (0x55)
...
int32 sd_card_update_clock(void)
{
  ...
  *SD0_CMDR = SD_CMD_START | SD_CMD_UPCLK_ONLY | SD_CMD_WAIT_PRE_OVER;
  ...
  while(timeout && (*SD0_CMDR & SD_CMD_START))
  {
    timeout--;
...
int32 sd_card_send_command(PSD_CARD_COMMAND command, PSD_CARD_DATA data)
{
  ...
  *SD0_CMDR = command->cmdidx | cmdval;
  ...
  if(command->resp_type & SD_RESPONSE_BUSY)
  {
    ...
    while(timeout && (*SD0_STAR & SD_STATUS_CARD_DATA_BUSY))
    {
      timeout--;
    ...
    case 0:
      {
        ...
        while(count)
        {
          ...
          while(timeout && (*SD0_STAR & status_bit))
          {
            timeout--;
...
int32 sd_rint_wait(uint32 timeout, uint32 status_bit)
{
  ...
  while(timeout && !(*SD0_RISR & status_bit))
  {
    timeout--;
    
    //Check on timeout or error
    if((timeout == 0) || (*SD0_RISR & SD_RINT_INTERRUPT_ERROR_BITS))
    {
      return(SD_ERROR_TIMEOUT);
    }
  }
