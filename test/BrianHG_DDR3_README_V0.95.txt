BrianHG_DDR3_README_V0.95.txt Status/Revision Log, Instructions.
...
Changes: BrianHG_DDR3_PLL.sv: Version 1.1, July 22, 2021
    ...
    The PLL now has 4 clock outputs plus the original parameter configurable CMD_CLK.
...
1) When running the complete controller at Quarter speed, a bug with consecutive sequential burst no longer have missing gaps.
...
BrianHG_DDR3_DECA_only_PHY_SEQ        -> Minimal project only initiating the DDR3 controller with RS232 debugger.  No multiport.        See top file: BrianHG_DDR3_DECA_test1_top.sv
BrianHG_DDR3_DECA_RS232_DEBUG_TEST    -> Minimal project initiating the complete DDR3 controller multiport system with RS232 debugger.  See top file: BrianHG_DDR3_DECA_top.sv
BrianHG_DDR3_DECA_Show_1080p          -> Project initiating the complete DDR3 controller with HDMI out showing a 1080p picture & RS232 Debugger.  See top file: BrianHG_DDR3_DECA_top.sv
BrianHG_DDR3_DECA_GFX_DEMO            -> Project initiating the complete DDR3 controller with a geometry drawing engine & HDMI out showing a 1080p picture & RS232 Debugger. *** Make sure you toggle the switches...  See top file: BrianHG_DDR3_DECA_top.sv
BrianHG_DDR3_GFX_source               -> .sv source files used with the demos projects which have HDMI video out.

    For the pll clocks:
        MAX10_CLK1_50 -> CLK_IN,      This is the source oscillator clock at 50MHz.
        BHG_*|pll1[0] -> DDR_CLK,     DDR3 clock and runs at the 300-400MHz.
        BHG_*|pll1[1] -> DDR_CLK_WDQ, DDR3 write data clock, set to 90 degrees out of phase compared to BHG_*|pll1[0].
        BHG_*|pll1[2] -> DDR_CLK_RDQ, DDR3 read data clock, at power-up, this clock is automatically tuned to the best phase to capture the read data coming back from the DDR3 ram chips.
        BHG_*|pll1[3] -> CMD_CLK,     This is the interface clock for the DDR3 controller and is called CMD_CLK throughout the controller's code. 
                                      When the parameter 'INTERFACE_SPEED' is set to 'FULL', this clock = BHG_*|pll1[0].
                                      When the parameter 'INTERFACE_SPEED' is set to 'HALF', this clock = 1/2 BHG_*|pll1[0].
                                      When the parameter 'INTERFACE_SPEED' is set to 'QUARTER', this clock = 1/4 BHG_*|pll1[0].
