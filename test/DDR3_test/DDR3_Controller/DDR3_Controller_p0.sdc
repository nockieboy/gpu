...
set data_input_max_delay [ DDR3_Controller_p0_round_3dp [ expr $t(rdu_input_max_delay_external) + $t(rdu_input_max_delay_internal) ]]
...
set data_input_min_delay [ DDR3_Controller_p0_round_3dp [ expr - $t(rdu_input_min_delay_external) - $t(rdu_input_min_delay_internal) ]]

# Minimum delay on address and command paths
set ac_min_delay [ DDR3_Controller_p0_round_3dp [ expr - $t(IH) -$fpga(tPLL_JITTER) - $fpga(tPLL_PSERR) - $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) - $ISI(addresscmd_hold) ]]

# Maximum delay on address and command paths
set ac_max_delay [ DDR3_Controller_p0_round_3dp [ expr $t(IS) +$fpga(tPLL_JITTER) + $fpga(tPLL_PSERR) + $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) + $ISI(addresscmd_setup) ]]
...
foreach { inst } $instances {
	...
	set fpga(tDQS_PHASE_JITTER) [ expr [ get_integer_node_delay -integer $::GLOBAL_DDR3_Controller_p0_dqs_delay_chain_length -parameters {IO MAX HIGH} -src DQS_PHASE_JITTER -in_fitter ] / 1000.0 ]
	...
	set final_data_input_max_delay [ DDR3_Controller_p0_round_3dp [ expr $data_input_max_delay + $fpga(tDQS_PSERR) ]]
	set final_data_input_min_delay [ DDR3_Controller_p0_round_3dp [ expr $data_input_min_delay - $t(CK) / 2.0 + $t(QH_time) - $fpga(tDQS_PSERR) - $tJITper]]

	if { $debug } {
		post_message -type info "SDC: Jitter Parameters"
		post_message -type info "SDC: -----------------"
		post_message -type info "SDC:    DQS Phase: $::GLOBAL_DDR3_Controller_p0_dqs_delay_chain_length"
	...
	# HR PHY clock
	set local_pll_afi_phy_clk [ DDR3_Controller_p0_get_or_add_clock_vseries \
	...
	# A/C clock
	set local_pll_addr_cmd_clk [ DDR3_Controller_p0_get_or_add_clock_vseries \
		-target $pll_ac_clock \
		-suffix "addr_cmd_clk" \
		-source $pll_ref_clock \
	...
	# NIOS clock
	set local_pll_avl_clock [ DDR3_Controller_p0_get_or_add_clock_vseries \
		-target $pll_avl_clock \
		-suffix "avl_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_DDR3_Controller_p0_pll_mult(PLL_NIOS_CLK) \
		-divide_by $::GLOBAL_DDR3_Controller_p0_pll_div(PLL_NIOS_CLK) \
		-phase $::GLOBAL_DDR3_Controller_p0_pll_phase(PLL_NIOS_CLK) ]

	set mem_factor [expr double($::GLOBAL_DDR3_Controller_p0_pll_mult(PLL_MEM_CLK)) / $::GLOBAL_DDR3_Controller_p0_pll_div(PLL_MEM_CLK)]
	set avl_factor [expr double($::GLOBAL_DDR3_Controller_p0_pll_mult(PLL_NIOS_CLK)) / $::GLOBAL_DDR3_Controller_p0_pll_div(PLL_NIOS_CLK)]
	set write_to_avl_clk_ratio [expr int($mem_factor / $avl_factor)]	
	

	# I/O scan chain clock
	set local_pll_config_clock [ DDR3_Controller_p0_get_or_add_clock_vseries \
		-target $pll_config_clock \
		-suffix "config_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_DDR3_Controller_p0_pll_mult(PLL_CONFIG_CLK) \
		-divide_by $::GLOBAL_DDR3_Controller_p0_pll_div(PLL_CONFIG_CLK) \
		-phase $::GLOBAL_DDR3_Controller_p0_pll_phase(PLL_CONFIG_CLK) ]	



	# Pulse-generator used by DQS tracking
	set local_sampling_clock "${inst}|DDR3_Controller_p0_sampling_clock"
	...
	set ac_fr_phase_mod_360 [expr round(2*$::GLOBAL_DDR3_Controller_p0_pll_phase(3)) % 360]
	set ck_fr_phase_mod_360 [expr round($::GLOBAL_DDR3_Controller_p0_pll_phase(1)) % 360]
...
if {(($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
	set dqs_clocks [DDR3_Controller_p0_get_all_instances_dqs_pins DDR3_Controller_p0_ddr_db]
