...
#############################################################
proc DDR3_Controller_p0_perform_flexible_write_launch_timing_analysis {opcs opcname inst family scale_factors_name interface_type max_package_skew dll_length period pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name} {
	...
	set msg_list [ list ]
	set dq_pll_clock_id [DDR3_Controller_p0_get_output_clock_id [ join [ join $pins(q_groups) ]] "DQ output" msg_list]
	...
	set pll_max [DDR3_Controller_p0_min_in_collection [get_path -rise_from ${pll_clock}|vco*ph[0] -rise_to ${pll_clock}|divclk] "arrival_time"]
	set pll_min [DDR3_Controller_p0_min_in_collection [get_path -rise_from ${pll_clock}|vco*ph[0] -rise_to ${pll_clock}|divclk -min_path] "arrival_time"]
	...
	if {($result == 1)} {
		...
		foreach dqpins $pins(q_groups) {
			...
			set msg_list [list]
			set leveling_delay_chain_name [DDR3_Controller_p0_traverse_to_leveling_delay_chain $dqspin msg_list]

			set dqs_periphery_node ${leveling_delay_chain_name}|clkin
			
			set cps_name [DDR3_Controller_p0_traverse_to_clock_phase_select $dqspin msg_list]
			...
			foreach dqpin $dqdmpins {
				...
				set msg_list [list]
				set leveling_delay_chain_name [DDR3_Controller_p0_traverse_to_leveling_delay_chain $dqpin msg_list]
				
				set dq_periphery_node ${leveling_delay_chain_name}|clkin

				set cps_name [DDR3_Controller_p0_traverse_to_clock_phase_select $dqpin msg_list]
...
proc DDR3_Controller_p0_perform_flexible_read_capture_timing_analysis {opcs opcname inst family scale_factors_name io_std interface_type max_package_skew dqs_phase period all_dq_pins pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name fpga_name} {
	...
	if {$IP(read_deskew_mode) == "dynamic"} {
		...
		# Consider variation in the delay chains used during dynamic deksew
		set dqs_period [ DDR3_Controller_p0_get_dqs_period $pins(dqs_pins) ]
		set offset_from_90 [DDR3_Controller_p0_get_acv_read_offset $period $dqs_phase $dqs_period]
		if {$IP(num_ranks) == 1} {
...
proc DDR3_Controller_p0_perform_ac_analyses {opcs opcname inst scale_factors_name pin_array_name timing_parameters_array_name summary_name IP_name} {
	...
	foreach ck_pin $pins(ck_pins) {
		set clk_clk_phase_select_node [DDR3_Controller_p0_traverse_to_clock_phase_select $ck_pin msg_list]
	...
	foreach ac_pin $ac_pins {
		...
		set ac_clk_phase_select_node [DDR3_Controller_p0_traverse_to_clkbuf $ac_pin msg_list]
		...
		set extra_ccpr [DDR3_Controller_p0_round_3dp [expr ($clk_max - $clk_min + 0.5*($ac_max - $ac_min))/2.0]]
		...
		set ACperiphery_min_delay [DDR3_Controller_p0_min_in_collection $ACperiphery_min "arrival_time"]
		set ACperiphery_max_delay [DDR3_Controller_p0_max_in_collection $ACperiphery_max "arrival_time"]
		set aiot_delay [DDR3_Controller_p0_round_3dp [expr [DDR3_Controller_p0_get_rise_aiot_delay [lindex $ac_pin 0]] * 1e9]]
	...
	lappend ac_summary [list "  Extra common clock and spatial correlation pessimism removal" [DDR3_Controller_p0_format_3dp [expr $setup_slack - $default_setup_slack]] [DDR3_Controller_p0_format_3dp [expr $hold_slack - $default_hold_slack]]]
	...
	foreach dqs_pin $pins(dqs_pins) {
		...
		set dqs_clk_phase_select_node [DDR3_Controller_p0_traverse_to_clock_phase_select $dqs_pin msg_list]
...
proc DDR3_Controller_p0_perform_flexible_bus_turnaround_time_analysis {opcs opcname instname family period dll_length interface_type tJITper tJITdty tDCD pll_steps pin_array_name timing_parameters_array_name summary_name  MP_name IP_name SSN_name board_name ISI_parameters_name} {
...
proc DDR3_Controller_p0_perform_resync_timing_analysis {opcs opcname inst fbasename family scale_factors_name io_std interface_type period pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name fpga_name SSN_name} {
	...
	set max_dqs_common_to_fifo [DDR3_Controller_p0_max_in_collection [get_path -from $dqs_pins -to $fifo -nworst 1] "arrival_time"]
...
proc DDR3_Controller_p0_perform_flexible_postamble_timing_analysis {opcs opcname instname scale_factors_name family period dll_length interface_type tJITper tJITdty tDCD DQSpathjitter pin_array_name timing_parameters_array_name summary_name  MP_name IP_name SSN_name board_name ISI_parameters_name} {
	...
	set mem_clock_delay_max [DDR3_Controller_p0_max_in_collection [get_path -rise_from $pins(pll_write_clock) -rise_to $pins(ck_pins)] "arrival_time"]
	set mem_clock_delay_min [DDR3_Controller_p0_min_in_collection [get_path -rise_from $pins(pll_write_clock) -rise_to $pins(ck_pins) -min_path] "arrival_time"]


	set dqs_delay_max  [DDR3_Controller_p0_max_in_collection [get_path -rise_from $pins(dqs_pins) -fall_to *POSTAMBLE_DFF] "arrival_time"]
	set dqs_delay_min  [DDR3_Controller_p0_min_in_collection [get_path -rise_from $pins(dqs_pins) -fall_to *POSTAMBLE_DFF -min_path] "arrival_time"]	
	set ck_pin_buffer_delay [DDR3_Controller_p0_round_3dp [expr [DDR3_Controller_p0_get_min_aiot_delay [lindex $pins(ck_pins) 0]] * 1e9]]
	...
	set clock_delay_max [DDR3_Controller_p0_max_in_collection [get_path -rise_from $pins(pll_write_clock) -rise_to $dqsenableextend_regs] "arrival_time"]
	set clock_delay_min [DDR3_Controller_p0_min_in_collection [get_path -rise_from $pins(pll_write_clock) -rise_to $dqsenableextend_regs -min_path] "arrival_time"]
	set t11_delay_max [DDR3_Controller_p0_max_in_collection [get_path -rise_from $dqsenableextend_regs -fall_to *POSTAMBLE_DFF] "arrival_time"]
	set t11_delay_min [DDR3_Controller_p0_min_in_collection [get_path -rise_from $dqsenableextend_regs -fall_to *POSTAMBLE_DFF -min_path] "arrival_time"]
